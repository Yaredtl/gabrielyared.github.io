...
const DOM = {
    ...
    cuenta: document.getElementById("particular"),
    inputRadios: document.querySelectorAll('input[type = radio]:checked'),
...
const validateInput = (input, errorSpan, minLength) => {
    input.addEventListener("change", () => {
        if (input.value.length < minLength) {
            errorSpan.textContent = `El campo debe tener al menos ${minLength} caracteres`;
        } else {
    ...
    validateInput(DOM.inputUsuario, DOM.errorUsuario, 4);
    validateInput(DOM.inputNombre, DOM.errorNombre, 2);
    validateInput(DOM.inputApellidos, DOM.errorApe, 2);
    validateInput(DOM.inputPostal, DOM.errorCod, 5);
    validateInput(DOM.inputTel, DOM.errorTele, 9);
    validateInput(DOM.contraseña, DOM.errorContra, 8);
    validateInput(DOM.descripcion, DOM.errorDesc, 10);
    validateInput(DOM.inputDni, DOM.errorDni, 9);
    validateInput(DOM.titulo, DOM.errorTitulo, 3);
    validateInput(DOM.fecha, DOM.errorNaci, 4);
    validateInput(DOM.aficiones, DOM.errorAfis, 2);
    validateInput(DOM.cuenta, DOM.errorCuenta, 1);
    //VALIDACIONES CON CHANGE

DOM.inputRadios.forEach(radio => {
    radio.addEventListener("change", () => {
        if (DOM.inputRadios.length > 0) {
